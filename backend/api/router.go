...
func NewRouter(client *storage.Client, service *app.Service, cfg *config.Config) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		v1.POST("/uploads/:bucket/resumable/init", h.InitResumableUpload)
		v1.GET("/uploads/:bucket/resumable/status", h.GetResumableUploadStatus)
		v1.PUT("/uploads/:bucket/resumable/part", h.UploadResumablePart)
		v1.POST("/uploads/:bucket/resumable/complete", h.CompleteResumableUpload)
		v1.DELETE("/uploads/:bucket/resumable", h.AbortResumableUpload)
